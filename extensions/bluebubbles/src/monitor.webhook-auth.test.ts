...
describe("BlueBubbles webhook monitor", () => {
  ...
  describe("webhook parsing + auth handling", () => {
    ...
    it("drops ambiguous mention-only direct payloads before dispatch", async () => {
      const account = createMockAccount();
      const config: OpenClawConfig = {};
      const core = createMockRuntime();
      setBlueBubblesRuntime(core);
    ...
    it("drops mention-only direct payloads when only group flag metadata is present", async () => {
      const account = createMockAccount();
      const config: OpenClawConfig = {};
      const core = createMockRuntime();
      setBlueBubblesRuntime(core);
    ...
    it("routes metadata-only group-hint payloads to group sessions", async () => {
      ...
      expect(mockResolveAgentRoute).toHaveBeenCalledWith(
        expect.objectContaining({
          peer: {
            kind: "group",
            id: "any;+;7a77739c144e46798b4747b98ebe63a4",
          },
        }),
      );
    });

    it("drops unresolved direct mirror metadata payloads before dispatch", async () => {
      const account = createMockAccount();
      const config: OpenClawConfig = {};
      const core = createMockRuntime();
      setBlueBubblesRuntime(core);
      ...
      const payload = {
        type: "new-message",
        data: {
          text: "metadata-only mirror event",
          handle: { address: "+15551234567" },
          isFromMe: false,
